#!/usr/bin/env python3
"""
Génération des 24 cartes de bascule via Gemini Flash
Cadavre Exquis des Futurs

Usage:
    export GOOGLE_API_KEY="AIza..."
    python3 generate_cards_gemini.py
"""
...
def generate_image_gemini_flash(api_key: str, card: dict, index: int) -> str:
    """Génère une image pour une carte via Gemini 2.0 Flash (génération d'image native)"""

    # Construire le prompt avec textes explicites
    full_prompt = f"""Generate an image of a portrait A5 card design for printing.

The card MUST display these exact French texts clearly and legibly:

TOP LEFT CORNER (small caps): "{card['category_label']}"
TOP CENTER (large bold title): "{card['title']}"
BOTTOM (medium description): "{card['text']}"

CENTER ILLUSTRATION:
{card['illustration']}

STYLE REQUIREMENTS:
- Retro-futuristic isometric illustration, clean lines, vintage infographic aesthetic
- Dominant color palette: {card['color']}
- Background: Aged textured cream paper (#F5F5F2)
- Typography: Bold geometric sans-serif for title, elegant serif for description
- Official document aesthetic from year 2036
- Portrait orientation (A5: 148mm x 210mm ratio)
- All French text must be spelled correctly and clearly readable"""

    print(f"\n[{index+1}/24] Génération: {card['title']}")
    print(f"   ID: {card['id']}")
    print(f"   Couleur: {card['color']}")

    # API Gemini 2.0 Flash avec génération d'image
    ...
    try:
        ...
        if 'candidates' in result and len(result['candidates']) > 0:
            ...
            for part in parts:
                if 'inlineData' in part:
                    img_base64 = part['inlineData'].get('data')
                    ...
                    if img_base64:
                        ...
                        img_path = OUTPUT_DIR / f"{card['id']}.{ext}"
        ...
        # Si pas d'image, afficher la réponse pour debug
        print(f"   ✗ Pas d'image trouvée dans la réponse")
        print(f"   Debug: {json.dumps(result, indent=2)[:500]}")
        return None

    except urllib.error.HTTPError as e:
        error_body = e.read().decode('utf-8') if e.fp else str(e)
        print(f"   ✗ Erreur HTTP {e.code}: {error_body[:500]}")
        return None
...
def main():
    ...
    if not api_key:
        ...
        print("Usage: export GOOGLE_API_KEY='AIza...' && python3 generate_cards_gemini.py")
        return

    print("=" * 60)
    print("Génération des 24 cartes de bascule")
    print("Cadavre Exquis des Futurs — Horizon 2036")
    print("Via Gemini 2.0 Flash")
    print("=" * 60)
    ...
    for i, card in enumerate(CARDS):
        result = generate_image_gemini_flash(api_key, card, i)
        if result:
